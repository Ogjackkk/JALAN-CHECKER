...
const Setting = () => {
  ...
  const [teacher, setTeacher] = useState(null); // <-- new state to hold teachers row
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      if (data?.user) {
        setUser(data.user);
        // fetch teacher profile row from "teachers" table using auth user id
        const { data: teacherData, error: teacherError } = await supabase
          .from('teachers')
          .select('id, username, teacher_id_number, email, created_at')
          .eq('id', data.user.id)
          .single();
  ...
  return (
    <div style={{ background: "#f7f7f7", minHeight: "100vh", maxWidth: "100vw", overflow: "hidden" }}>
      ...
      {/* Mobile menu overlay with updated styling */}
      {mobileMenuOpen && (
            ...
            <div className="profile-section">
              ...
              {user ? (
                <div style={{
                  ...
                  {/* show username and teacher id number from teachers table if available */}
                  <div style={{ marginBottom: "12px" }}>
                    <strong style={{ color: "#388e3c" }}>Username:</strong>
                    <span style={{ marginLeft: "8px", color: "#333" }}>{teacher?.username ?? "N/A"}</span>
                  </div>
                  <div style={{ marginBottom: "12px" }}>
                    <strong style={{ color: "#388e3c" }}>Teacher ID:</strong>
                    <span style={{ marginLeft: "8px", color: "#333" }}>{teacher?.teacher_id_number ?? "N/A"}</span>
                  </div>
                  <div style={{ marginBottom: "12px" }}>
                    <strong style={{ color: "#388e3c" }}>Email:</strong>
                    <span style={{ marginLeft: "8px", color: "#333" }}>{teacher?.email ?? user.email}</span>
                  </div>
